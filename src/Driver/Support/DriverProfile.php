...
final class DriverProfile
{
    ...
    public static function applyConnectionDefaults(array $config): array
    {
        ...
        if (!is_string($driver) || $driver === '') { return $config; }
        try { return DriverRegistry::resolve($driver)->mergeDefaults($config); } catch (ConnectionException) { return $config; }
    }
    ...
    private static function causedByDriverClassifier(string $driver, Throwable $e, callable $messageClassifier, callable $metadataClassifier): bool
    {
        ...
        if ($messageClassifier($driver, $e->getMessage())) { return true; }
        return $e instanceof PDOException && $metadataClassifier($driver, $e);
    }
    /** @param list<string> $knownCodes */
    private static function matchesKnownDeadlockCode(?string $code, array $knownCodes): bool { return $code !== null && $code !== '' && in_array($code, $knownCodes, true); }
    ...
    private static function metadataSuggestsDeadlock(string $driver, PDOException $e): bool { return self::metadataMatchesStateAndCodeSets($e, self::DEADLOCK_SQLSTATES[$driver] ?? [], self::DEADLOCK_ERROR_CODES[$driver] ?? []); }
    private static function metadataSuggestsRetryableTransactionConflict(string $driver, PDOException $e): bool { return self::metadataMatchesStateAndCodeSets($e, self::RETRYABLE_TX_SQLSTATES[$driver] ?? [], self::RETRYABLE_TX_ERROR_CODES[$driver] ?? []); }
    private static function nullableString(mixed $value): ?string { return is_scalar($value) ? (string) $value : null; }
}
