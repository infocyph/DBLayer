...
final class Transaction
{
    ...
    /**
     * @var array{total:int,committed:int,rolled_back:int,deadlocks:int,in_transaction:bool,current_level:int,savepoints:int,elapsed_time:float}
     */
    private array $stats = [
        'total' => 0,
        'committed' => 0,
    ...
    /** @return array{total:int,committed:int,rolled_back:int,deadlocks:int,in_transaction:bool,current_level:int,savepoints:int,elapsed_time:float} */
    ...
    private function createSavepoint(int $level): void
    {
        ...
        $this->connection->statement(
            DriverProfile::createSavepointSql($this->connection->getDriverName(), $savepoint),
        );
    }
    ...
    private function releaseSavepoint(int $level): void
    {
        ...
        $sql = DriverProfile::releaseSavepointSql(
    ...
    private function rollbackToSavepoint(int $level): void
    {
        ...
        $this->connection->statement(
            DriverProfile::rollbackToSavepointSql(
                $this->connection->getDriverName(),
                'trans_' . $level,
            ),
        );
    ...
    private function transactionDurationMs(): float
    {
        return $this->startedAt === null
            ? 0.0
            : (microtime(true) - $this->startedAt) * 1_000.0;
    }
}
