...
final class Transaction
{
    ...
    public function commit(): void
    {
        ...
        if (!$this->connection->commitNativeTransaction()) {
            throw TransactionException::commitFailed($this->nativeErrorMessage($this->connection->getPdo(), 'The PDO driver returned false.'));
        }
        ...
        Events::dispatch('db.transaction.committed', [new TransactionCommitted($this->connection, $durationMs)]);
    ...
    public function rollBack(): void
    {
        ...
        if (!$this->connection->rollBackNativeTransaction()) {
            throw TransactionException::rollBackFailed($this->nativeErrorMessage($this->connection->getPdo(), 'The PDO driver returned false.'));
        }
        $durationMs = $this->transactionDurationMs();
    ...
    private function beginTopLevel(): void
    {
        if (!$this->connection->beginNativeTransaction()) {
            throw TransactionException::beginFailed($this->nativeErrorMessage($this->connection->getPdo(), 'The PDO driver returned false.'));
        }
    }

    private function causedByRetryableTransactionError(Throwable $e): bool
    {
        return DriverProfile::causedByRetryableTransactionError($this->connection->getDriverName(), $e);
    }

    private function createSavepoint(int $level): void
    {
        if (!$this->connection->getCapabilities()->supportsSavepoints) {
            throw TransactionException::failed(new \LogicException('Nested transactions require driver savepoint support.'));
        }
        $savepoint = 'trans_' . $level;
        $this->connection->statement(DriverProfile::createSavepointSql($this->connection->getDriverName(), $savepoint));
    }
    ...
    private function promoteAfterCommitCallbacks(int $fromLevel, int $toLevel): void
    {
        ...
        if ($callbacks !== []) {
            $this->afterCommitCallbacks[$toLevel] = [...($this->afterCommitCallbacks[$toLevel] ?? []), ...$callbacks];
        }
    }
    ...
    private function releaseSavepoint(int $level): void
    {
        ...
        $sql = DriverProfile::releaseSavepointSql($this->connection->getDriverName(), 'trans_' . $level);
        if ($sql !== null) {
    ...
    {
        return $this->startedAt === null ? 0.0 : (microtime(true) - $this->startedAt) * 1_000.0;
    }
}
