...
final readonly class SchemaColumnCompiler
{
    ...
    {
        $this->dialect = is_string($dialect)
            ? SchemaDialectRegistry::resolve($dialect)
            : $dialect;
    }
    ...
    public function compile(ColumnDefinition $column): string
    {
        ...
        $sql = $this->wrap($column->name)
            ...
            . ($column->autoIncrement && $this->dialect->isSqlServer() ? ' IDENTITY(1,1)' : '')
    ...
    public function compileType(ColumnDefinition $column): string
    {
        ...
        return match ($column->type) {
            ...
            'boolean' => match (true) {
                $this->dialect->isMySqlFamily() => 'TINYINT(1)',
                $this->dialect->isSqlServer() => 'BIT',
                default => 'BOOLEAN',
            },
            ...
            'ulid' => $this->dialect->isSqlite() ? 'TEXT' : 'CHAR(26)',
            'uuid' => $this->uuidType(),
    ...
    {
        return match (true) {
            $this->dialect->isPostgreSql() => 'BYTEA',
            $this->dialect->isSqlServer() => 'VARBINARY(MAX)',
            default => 'BLOB',
        };
    }
    ...
    private function choiceType(ColumnDefinition $column, bool $set): string
    {
        ...
        return match (true) {
            $this->dialect->isSqlite() => 'TEXT',
            $this->dialect->isSqlServer() => 'NVARCHAR(255)',
            default => 'VARCHAR(255)',
        };
    }
    ...
    private function generated(ColumnDefinition $column): string
    {
        ...
        if ($this->dialect->isSqlServer()) {
            return sprintf(
                ...
                $column->generatedStorage === 'stored' ? ' PERSISTED' : '',
            );
        }
    ...
    private function integerType(ColumnDefinition $column): string
    {
        ...
        return match ($column->type) {
            'bigInteger' => 'BIGINT',
            'mediumInteger' => $this->dialect->isMySqlFamily() ? 'MEDIUMINT' : 'INTEGER',
            'smallInteger' => 'SMALLINT',
            'tinyInteger' => $this->dialect->isMySqlFamily() || $this->dialect->isSqlServer() ? 'TINYINT' : 'SMALLINT',
            default => 'INTEGER',
        };
    }
    ...
    private function numericType(ColumnDefinition $column): string
    {
        ...
        return $column->type === 'double' && $this->dialect->isPostgreSql()
            ? 'DOUBLE PRECISION'
    ...
    private function stringType(ColumnDefinition $column): string
    {
        ...
        if ($this->dialect->isSqlServer()) {
            return match ($column->type) {
                'char' => sprintf('NCHAR(%d)', $column->length ?? throw SchemaException::invalid('Character length is required.')),
                'string' => sprintf('NVARCHAR(%d)', $column->length ?? throw SchemaException::invalid('String length is required.')),
                default => 'NVARCHAR(MAX)',
            };
        }

        return match ($column->type) {
            'char' => sprintf('CHAR(%d)', $column->length ?? throw SchemaException::invalid('Character length is required.')),
            'longText' => $this->dialect->isMySqlFamily() ? 'LONGTEXT' : 'TEXT',
            'mediumText' => $this->dialect->isMySqlFamily() ? 'MEDIUMTEXT' : 'TEXT',
            'string' => sprintf('VARCHAR(%d)', $column->length ?? throw SchemaException::invalid('String length is required.')),
            'tinyText' => $this->dialect->isMySqlFamily() ? 'TINYTEXT' : 'TEXT',
            default => 'TEXT',
        };
    }
    ...
    private function temporalType(string $mysql, string $pgsql, ColumnDefinition $column): string
    {
        ...
        if ($this->dialect->isSqlServer()) {
            return match ($column->type) {
                'dateTime', 'timestamp' => 'DATETIME2' . $precision,
                'dateTimeTz', 'timestampTz' => 'DATETIMEOFFSET' . $precision,
                'time' => 'TIME' . $precision,
                'timeTz' => throw SchemaException::unsupported($this->dialect->name(), 'time with timezone column'),
    ...
    {
        return match (true) {
            $this->dialect->isPostgreSql() => 'UUID',
            $this->dialect->isMySqlFamily() => 'CHAR(36)',
            $this->dialect->isSqlServer() => 'UNIQUEIDENTIFIER',
            default => 'TEXT',
        };
    }
    ...
    private function validateAutoIncrement(ColumnDefinition $column): void
    {
        if (
            ...
            && !in_array($column->type, ['tinyInteger', 'smallInteger', 'mediumInteger', 'integer', 'bigInteger'], true)
        ) {
    ...
    {
        return match (true) {
            $this->dialect->isMySqlFamily() => 'YEAR',
            $this->dialect->isPostgreSql(), $this->dialect->isSqlServer() => 'SMALLINT',
            default => 'INTEGER',
        };
    }
}
