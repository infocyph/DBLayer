...
final readonly class SchemaManager
{
    ...
    {
        $this->grammar = new SchemaGrammar($connection->getDriverName(), $connection->getTablePrefix());
    }

    public function connection(): Connection { return $this->connection; }
    public function create(string $table, callable $definition): void { $this->execute($this->build($table, true, $definition)); $this->invalidateTables([$table]); }
    public function drop(string $table): void { $this->connection->statement($this->grammar->compileDrop($table)); $this->invalidateTables([$table]); }

    public function dropAllTables(bool $authorized = false): void
    {
        if (!$authorized) { throw MigrationException::destructiveDenied('drop-all-tables'); }
        $tables = $this->tables();
        if ($tables === []) { return; }
        foreach ($this->grammar->beforeDropAllStatements() as $statement) { $this->connection->statement($statement); }
        try {
            foreach ($tables as $table) {
                $this->connection->statement($this->grammar->compileDrop($table, true) . $this->grammar->dropTableSuffix());
            }
        } catch (Throwable $primary) {
            try { $this->restoreForeignKeyChecks(); } catch (Throwable $cleanup) { throw MigrationException::cleanupAlsoFailed($primary, $cleanup); }
            throw $primary;
        }
    ...
    public function dropIfExists(string $table): void { $this->connection->statement($this->grammar->compileDrop($table, true)); $this->invalidateTables([$table]); }
    ...
    public function supportsTransactionalDdl(): bool { return $this->grammar->supportsTransactionalDdl(); }
    public function table(string $table, callable $definition): void { $this->execute($this->build($table, false, $definition)); $this->invalidateTables([$table]); }
    ...
    public function toSql(string $table, bool $creating, callable $definition): array { return $this->build($table, $creating, $definition); }
    /** @return non-empty-list<string> */
    ...
    {
        $parts = explode('.', $table); if (count($parts) === 1) { return [null, $parts[0]]; }
        $name = array_pop($parts); return [implode('.', $parts), $name];
    }
}
