...
{
    #[\Override] public function name(): string { return 'mssql'; }
    #[\Override] public function isSqlServer(): bool { return true; }
    ...
    #[\Override] public function compileAddColumn(string $table, string $definition): string { return sprintf('ALTER TABLE %s ADD %s', $this->wrap($table), $definition); }
    #[\Override] public function compileRenameTable(string $from, string $to): string
    {
        $fromParts = explode('.', $from); $toParts = explode('.', $to);
        ...
        if ($fromSchema !== $toSchema) { throw SchemaException::unsupported('mssql', 'rename table across schemas'); }
        return sprintf("EXEC sp_rename N'%s', N'%s'", str_replace("'", "''", $from), str_replace("'", "''", (string) end($toParts)));
    }
    #[\Override] public function compileRenameColumn(string $table, string $from, string $to): string
    {
    ...
    #[\Override] public function compileRenameIndex(string $table, string $from, string $to): string
    {
